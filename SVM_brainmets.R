...
for (i in 1:5) {
  ...
  # Train SVM model using the balanced training dataset
  model <- svm(
    x = features_balanced,
    y = target_balanced,
  ...
  predictions <- predict(model, features_test, probability = TRUE)
  predicted_probs <- attr(predictions, "probabilities")[, 2]
...
write.xlsx(shap_summary, file = "shap_summary.xlsx", rowNames = FALSE)

# Save the SHAP values to an Excel sheet
write.xlsx(shap_values, file = "shap_values.xlsx", rowNames = FALSE)
