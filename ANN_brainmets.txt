...
for (i in 1:5) {
  ...
  # Convert target to numeric for neuralnet
  target_balanced <- as.numeric(target_balanced) - 1
  target_test <- as.numeric(target_test) - 1

  # Train the neural network using the balanced training dataset
  formula <- as.formula(paste("target_balanced ~", paste(names(features_balanced), collapse = " + ")))
  model <- neuralnet(
    formula,
    data = data.frame(features_balanced, target_balanced),
    hidden = 10,  # One layer with 10 neurons
    linear.output = FALSE,
    stepmax = 1e6
  )
  models[[i]] <- model
  ...
  predicted_probs <- compute(model, features_test)$net.result
  predicted_classes <- as.factor(ifelse(predicted_probs > 0.5, 1, 0))

  # Assuming the positive class is 1 and negative class is 0
  positive_class <- 1
  negative_class <- 0
  ...
  confusion_matrix <- confusionMatrix(predicted_classes, target_test, positive = as.character(positive_class))
...
predict_function <- function(model, newdata) {
  probs <- compute(model, newdata)$net.result
  matrix(probs, ncol = 1)
}
...
write.xlsx(shap_summary, file = "ANNshap_summary.xlsx", rowNames = FALSE)

# Save the SHAP values to an Excel sheet
write.xlsx(shap_values, file = "ANNshap_values.xlsx", rowNames = FALSE)
