...
for (i in 1:5) {
  ...
  # Train GLMnet model using the balanced training dataset
  model <- cv.glmnet(
    x = as.matrix(features_balanced),
    y = as.numeric(target_balanced) - 1,
    family = "binomial",
  ...
  predicted_probs <- predict(model, newx = as.matrix(features_test), type = "response", s = "lambda.min")
...
predict_function <- function(model, newdata) {
  probs <- predict(model, newx = as.matrix(newdata), type = "response", s = "lambda.min")
  matrix(probs, ncol = 1)
}
...
write.xlsx(shap_summary, file = "GLMnetshap_summary.xlsx", rowNames = FALSE)

# Save the SHAP values to an Excel sheet
write.xlsx(shap_values, file = "GLMnetshap_values.xlsx", rowNames = FALSE)
