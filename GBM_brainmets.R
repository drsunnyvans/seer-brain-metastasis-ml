...
for (i in 1:5) {
  ...
  # Convert the balanced dataset to a DMatrix object for xgboost
  dtrain <- xgb.DMatrix(data = as.matrix(features_balanced), label = as.numeric(target_balanced) - 1)

  # Train xgboost model using the balanced training dataset
  params <- list(
  ...
  dtest <- xgb.DMatrix(data = as.matrix(features_test))
...
predict_function <- function(model, newdata) {
  dnewdata <- xgb.DMatrix(data = as.matrix(newdata))
  probs <- predict(model, dnewdata)
  matrix(probs, ncol = 1)
}
...
write.xlsx(shap_summary, file = "GBMshap_summary.xlsx", rowNames = FALSE)

# Save the SHAP values to an Excel sheet
write.xlsx(shap_values, file = "GBMshap_values.xlsx", rowNames = FALSE)
