...
# Read and check data
data <- read_excel("Final Table2-Encoding.xlsx")
...
for (i in 1:5) {
  ...
  # Train GLM model using the balanced training dataset
  model <- glm(
    target_balanced ~ ., 
  ...
  predicted_probs <- predict(model, newdata = features_test, type = "response")
...
write.xlsx(shap_summary, file = "GLMshap_summary.xlsx", rowNames = FALSE)

# Save the SHAP values to an Excel sheet
write.xlsx(shap_values, file = "GLMshap_values.xlsx", rowNames = FALSE)
